...
@test "target directory starts with a hyphen is rejected" {
    run "$INSTALL_SCRIPT" "-my-target-dir"
    [ "$status" -eq 1 ]
    [[ "$output" == *"Error: Installation target directory cannot start with a hyphen"* ]]
}
