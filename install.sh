    ...
    case $1 in
        ...
        *) 
            if [[ -z "$TARGET_DIR" ]]; then
                TARGET_DIR="$1"
                # Security: Validate that the target directory path does not start with a hyphen to prevent option injection
                [[ "$TARGET_DIR" =~ ^- ]] && error_exit "Target directory path cannot start with a hyphen: $TARGET_DIR"
            else
