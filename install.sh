    ...
    case $1 in
        ...
        *) 
            if [[ -z "$TARGET_DIR" ]]; then
                TARGET_DIR="$1"
                # Security: Validate target directory path to prevent option injection
                [[ "$TARGET_DIR" =~ ^- ]] && error_exit "Installation target directory cannot start with a hyphen: $TARGET_DIR"
            else
