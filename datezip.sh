    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            --dest)
                ...
                [[ "$RESTORE_DEST" =~ ^- ]] && { echo "Error: --dest cannot start with a hyphen to prevent option injection" >&2; exit 1; }
                ;;
            --files)
                RESTORE_FILES="$2"; shift
                [[ "$RESTORE_FILES" =~ ^- ]] && { echo "Error: --files cannot start with a hyphen to prevent option injection" >&2; exit 1; }
                ;;
