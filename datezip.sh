    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            --files)
                ...
                IFS=',' read -ra check_files <<< "$RESTORE_FILES"
                for check_f in "${check_files[@]}"; do
                    [[ "$check_f" == -* ]] && { echo "Error: file path cannot start with a hyphen: $check_f" >&2; exit 1; }
                done
                ;;
...
execute_status() {
    ...
    if [[ -n "$common" ]]; then
        ...
        while IFS= read -r f; do
            [[ -z "$f" ]] && continue
            # Get latest mtime from cache - using fixed-string awk to prevent regex/option injection
