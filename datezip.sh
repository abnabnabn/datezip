    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            --dest)
                ...
                # Security: Prevent option/argument injection by rejecting paths starting with a hyphen
                [[ "$RESTORE_DEST" =~ ^- ]] && { echo "Error: --dest cannot start with a hyphen" >&2; exit 1; }
                ;;
            --files)
                RESTORE_FILES="$2"; shift
                # Security: Prevent option/argument injection by rejecting file lists starting with a hyphen
