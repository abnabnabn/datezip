    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            --dest)
                ...
                # Security: Validate destination directory to prevent option injection
                [[ "$RESTORE_DEST" =~ ^- ]] && { echo "Error: --dest cannot start with a hyphen" >&2; exit 1; }
                ;;
            --files)
                RESTORE_FILES="$2"; shift
                # Security: Validate each file in the comma-separated list to prevent option injection
                IFS=',' read -ra test_files <<< "$RESTORE_FILES"
                for f in "${test_files[@]}"; do
                    [[ "$f" =~ ^- ]] && { echo "Error: --files items cannot start with a hyphen" >&2; exit 1; }
                done
                ;;
...
execute_status() {
    ...
    if [[ -n "$common" ]]; then
        ...
        while IFS= read -r f; do
            [[ -z "$f" ]] && continue
            # Get latest mtime from cache - anchored to start of line using safe awk comparison instead of unescaped regex grep
            local cached_mtime=$(awk -F'|' -v f="$f" '$1 == f {print $2}' "$tmp_latest")
            # Get current mtime
