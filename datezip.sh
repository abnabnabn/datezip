    ...
    while [[ "$#" -gt 0 ]]; do
        case $1 in
            ...
            --files)
                ...
                IFS=',' read -ra file_list <<< "$RESTORE_FILES"
                for f in "${file_list[@]}"; do
                    [[ "$f" =~ ^- ]] && { echo "Error: --files elements cannot start with a hyphen" >&2; exit 1; }
                done
                ;;
...
execute_status() {
    ...
    if [[ -n "$common" ]]; then
        ...
        while IFS= read -r f; do
            [[ -z "$f" ]] && continue
            # Get latest mtime from cache using fixed-string comparison to avoid regex/option injection
